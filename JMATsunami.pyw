...
# url = "http://www.yoyo0901.byethost16.com/津波警報/2-32-39_11_02_120615_VTSE41.xml"#測試用(單資料警報)
# url = "http://www.yoyo0901.byethost16.com/津波警報/3-32-39_11_02_120615_VTSE41.xml"#測試用(單資料警報2)
        ...
        for i in range(3): #重複3次 判定大海嘯、海嘯警報、海嘯注意等
            ...
            for i in tsunami["Forecast"]["Item"]:
                if type(i) != str:
                    ...
                    if (tsunami["Forecast"]["Item"]["Category"]["Kind"]["Name"]).replace("大津波警報：発表","大津波警報") == warningname[l]:
            ...
            for f in range(3):
                for i in ab:
                    if type(i) != str:
                        ...
                        try:
                            if warning[j] == tsunami["Forecast"]["Item"]["Category"]["Kind"]["Name"].replace("：発表",""):
                                output = "　" + tsunami["Forecast"]["Item"]["Area"]["Name"]                  
                                if tsunami["Forecast"]["Item"]["FirstHeight"]["Condition"] == Condition[f]:
                                    output = ""
                                    warn = (tsunami["Forecast"]["Item"]["Category"]["Kind"]["Name"]).replace("：発表","") #警報種類
                                    areaname = tsunami["Forecast"]["Item"]["Area"]["Name"] #預報區

                                    try: #預測到達時間
                                        arrtime = tsunami["Forecast"]["Item"]["FirstHeight"]["Condition"] #此區為即將到達及第1波到達確認
                                    except:
                                        pass
            ...
            for i in ab:
                if type(i) != str:
                    ...
                    if warning[j] == tsunami["Forecast"]["Item"]["Category"]["Kind"]["Name"].replace("：発表",""):
                        ...
                        try: #預測到達時間
                            ampm = ""
                            arrtime = tsunami["Forecast"]["Item"]["FirstHeight"]["ArrivalTime"] #此區為正常預測時間(包含年月日時分秒等)
